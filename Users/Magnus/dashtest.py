...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Input(id="search-input", type="text", value=SEARCHPAGE),
...
def update_output(search_input):
    S = requests.Session()
    URL = "https://www.wikidata.org/w/api.php?action=wbsearchentities&search=%s&format=json&limit=5&formatversion=2&language=en" % (search_input)
