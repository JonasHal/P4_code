...
def readData():
    ...
    for i in file1:
        ...
        if (i.find('cannot_be_together') != -1):
            ...
            cannotBeTogether = i.replace(' ', '').replace('cannot_be_together:', '').replace('{', '[').replace('}',
                                                                                                               ']').replace(
                "'", 'a').rstrip().split('a')
    ...
    rel_path_data = "../Data/transaction.txt"
        ...
        if (ICount[L[l]] / TCount) >= MS[L[l]]:
            for h in range(l + 1, len(L)):
                if (ICount[L[h]] / TCount) >= MS[L[l]] and abs(
                        (ICount[L[h]] / TCount) - (ICount[L[l]] / TCount)) <= phi:
                    CList[2].append(list())
...
def MSCandidateGeneration(n):
    ...
    for i in range(0, len(FList[m])):
        for j in range(0, len(FList[m])):
            ...
            if (k == m - 1):
                if ((MISSortDict[FList[m][i][k]] < MISSortDict[FList[m][j][k]]) and (
                        abs((ICount[FList[m][i][k]] / TCount) - (ICount[FList[m][j][k]] / TCount)) <= phi)):
                    ...
                    for sub in range(0, len(subset)):
                        if (not CList[m + 1]):
                            if ((bool(CList[m + 1][len(CList[m + 1]) - 1][0]) in subset[sub]) or (
                                    MS[CList[m + 1][len(CList[m + 1]) - 1][1]] == MS[
                                CList[m + 1][len(CList[m + 1]) - 1][0]])):
...
def output():
    ...
    rel_path_results = Path("../Data/result1-1.txt")
